...
async def get_start_cashier(message: Message):
    print("Boshlanish", simple)
    if len(simple) == 1:
        if list_name == []:
            if 3 <= len(message.text.split()) <= 4:
                list_name.append(message.text)
                await message.answer("Tug'ilgan kuningizni kiriting 📅 (masalan, <b>02.11.1995</b>):")
                simple.append(2)
